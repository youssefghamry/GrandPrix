...
include GRANDPRIX_MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( GRANDPRIX_MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( grandprix_mikado_is_customizer_item_enabled( $module_load, 'grandprix_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'grandprix_mikado_get_icon_pack_collections' ) ) {
	function grandprix_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'grandprix_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'grandprix_mikado_get_icon_pack_options' ) ) {
	function grandprix_mikado_get_icon_pack_options() {
		$options = apply_filters( 'grandprix_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'grandprix_mikado_get_icon_pack_social_options' ) ) {
	function grandprix_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'grandprix_mikado_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: GrandPrixMikadoClassIconCollections
 */
class GrandPrixMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = grandprix_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = grandprix_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = grandprix_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = grandprix_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'grandprix' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'grandprix' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'grandprix' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'grandprix' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'grandprix' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'grandprix' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'grandprix' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'grandprix' )       => 'fa-lg',
			esc_html__( 'Small', 'grandprix' )      => 'fa-2x',
			esc_html__( 'Medium', 'grandprix' )     => 'fa-3x',
			esc_html__( 'Large', 'grandprix' )      => 'fa-4x',
			esc_html__( 'Very Large', 'grandprix' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iGrandPrixMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'grandprix' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'grandprix' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'grandprix-mikado-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
