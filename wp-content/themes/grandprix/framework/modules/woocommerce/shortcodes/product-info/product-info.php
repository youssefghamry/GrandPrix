<?php
namespace GrandPrixCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'grandprix' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'grandprix' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'grandprix' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'grandprix' ),
							'value'       => array(
								esc_html__( 'Title', 'grandprix' )          => 'title',
								esc_html__( 'Featured Image', 'grandprix' ) => 'featured_image',
								esc_html__( 'Category', 'grandprix' )       => 'category',
								esc_html__( 'Excerpt', 'grandprix' )        => 'excerpt',
								esc_html__( 'Price', 'grandprix' )          => 'price',
								esc_html__( 'Rating', 'grandprix' )         => 'rating',
								esc_html__( 'Add To Cart', 'grandprix' )    => 'add_to_cart',
								esc_html__( 'Tag', 'grandprix' )            => 'tag',
								esc_html__( 'Author', 'grandprix' )         => 'author',
								esc_html__( 'Date', 'grandprix' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'grandprix' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'grandprix' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'grandprix' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'grandprix' ),
							'value'       => array_flip( grandprix_mikado_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'grandprix' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'grandprix' ),
							'value'      => array(
								esc_html__( 'Default', 'grandprix' ) => '',
								esc_html__( 'White', 'grandprix' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'grandprix' ),
							'value'      => array(
								esc_html__( 'Default', 'grandprix' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'grandprix' ) ),
							'group'      => esc_html__( 'Product Info Style', 'grandprix' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="mkdf-pi-title entry-title" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="mkdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="mkdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="mkdf-pi-excerpt"><p itemprop="description" class="mkdf-pi-excerpt-item" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="mkdf-pi-price" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="mkdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "grandprix" ), $average ) . '" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'grandprix_mikado_filter_product_info_add_to_cart_link',
			sprintf( '<a rel="nofollow" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="%s">%s</a>',
				...
				esc_html__('Add To Cart', 'grandprix')
			),
			$product );
		$html .= '</div>';
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="mkdf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="mkdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="mkdf-pi-author" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="mkdf-pi-date" ' . grandprix_mikado_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'grandprix' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'grandprix' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'grandprix' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'grandprix' ) . ': ' . $product_id;
